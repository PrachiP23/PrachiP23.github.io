...
adobeAnalytics = {
	...
	*/
	lkup: {
		...
		ca_epp			: function() { return { rs:'lenovocapub,lenovoglobal',		cc:'ca', bu:'epp'			} },	// 140513
		ca_affinity		: function() { return { rs:'lenovocapub,lenovoglobal',	cc:'ca', bu:'affinity'		} },	// 140513
		ca_cepp			: function() { return { rs:'lenovocapub,lenovoglobal',	cc:'ca', bu:'cepp'			} },	// 140513
		ca_cpp			: function() { return { rs:'lenovocapub,lenovoglobal',	cc:'ca', bu:'cpp'			} },	// 140513
		ca_gepp			: function() { return { rs:'lenovocapub,lenovoglobal',	cc:'ca', bu:'gepp'			} },	// 140513
		ca_mpp			: function() { return { rs:'lenovocapub,lenovoglobal',	cc:'ca', bu:'mpp'			} },	// 140513
		ca_tele			: function() { return { rs:'lenovocapub,lenovoglobal',		cc:'ca', bu:'tele'			} },	// 140512
		ca_lpn			: function() { return { rs:'lenovocapub,lenovoglobal',		cc:'ca', bu:'lpn'			} },	// 140513
		/* us - United States */
		...
		us_affinity		: function() { return { rs:'lenovouspub,lenovoglobal',	cc:'us', bu:'affinity'		} },	// 140513
		us_epp			: function() { return { rs:'lenovouspub,lenovoglobal',		cc:'us', bu:'epp'			} },	// 140513
		us_eppdev		: function() { return { rs:'lenovoamericasdev',	cc:'us', bu:'eppdev'		} },	// 140513
		us_cepp			: function() { return { rs:'lenovouspub,lenovoglobal',	cc:'us', bu:'epp'			} },	// 140513
		us_ceppdev		: function() { return { rs:'lenovoamericasdev',	cc:'us', bu:'ceppdev'		} },	// 140513
		us_cpp			: function() { return { rs:'lenovouspub,lenovoglobal',	cc:'us', bu:'cpp'			} },	// 140513
		us_cppdev		: function() { return { rs:'lenovoamericasdev',	cc:'us', bu:'cppdev'		} },	// 140513
		us_mpp			: function() { return { rs:'lenovouspub,lenovoglobal',	cc:'us', bu:'mpp'			} },	// 140513
		us_mppdev		: function() { return { rs:'lenovoamericasdev',	cc:'us', bu:'mppdev'		} },	// 140513
		us_gepp			: function() { return { rs:'lenovouspub,lenovoglobal',	cc:'us', bu:'gepp'			} },	// 140513
		...
		us_lpn			: function() { return { rs:'lenovouspub,lenovoglobal',		cc:'us', bu:'lpn'			} },	// 140513
		us_outlet		: function() { return { rs:'lenovouspub,lenovoglobal',	cc:'us', bu:'outlet'		} },	// 140513
		us_tele			: function() { return { rs:'lenovouspub,lenovoglobal',		cc:'us', bu:'tele'			} },	// 140513
		us_smb			: function() { return { rs:'lenovouspub,lenovoglobal',		cc:'us', bu:'smb'			} },	// 140513
		us_ovp			: function() { return { rs:'lenovoglobal',		cc:'us', bu:'ovp'			} },	// 140521
		us_retail		: function() { return { rs:'lenovouspub,lenovoglobal',		cc:'us', bu:'retail'		} },	// 140820
		...
		ww_filter		: function() { return { rs:'lenovoglobaldev',	cc:'ww', bu:'filter'		} },	// 150724
		ww_forum		: function() { return { rs:'lenovoglobal',		cc:'ww', bu:'forum'			} },	// 150730
		ww_community	: function() { return { rs:'lenovoglobal',		cc:'ww', bu:'community'			} },	// 150824
...
/* Visitor ID Handling Functions */
_satellite.customGetVisitorID = function(useCookieFormat){
	try{
		var dtmVisitorID =_satellite.getVisitorId();
		var aID = dtmVisitorID.getAnalyticsVisitorID();
		if( typeof aID !== "undefined" && aID !== "" ){
			...
			var mcID = dtmVisitorID.getMarketingCloudVisitorID();
...
_satellite.backup_s_c_r = window.s_c_r();
window.s_c_r = function( cookieName ){
	try{
		if( typeof cookieName !== "undefined" && cookieName.match( /s_vi/i ) ){
			return _satellite.customGetVisitorID(true);
		} else {
			return window.backup_s_c_r(cookieName);
		}
	} catch(e){
	}
}
...
window.s_checkCustomRS = function() {
	...
	if( s_getQueryStr('setCustomRS') !== "" ){
		var queryCC = s_getQueryStr('cc').replace(/\W/g,""), queryBU = s_getQueryStr('bu').replace(/\W/g,""), queryStore = s_getQueryStr('store').replace(/\W/g,"");
		if( queryCC !== "" && queryBU !== "" ){
			var date = new Date();
	...
	/* Check for the resetSynthTraffic flag. If present, delete the s_traffic cookie */
	try{
		if( s_getQueryStr('resetSynthTraffic') !== "" ){
			var delDate = new Date();
			delDate.setTime(delDate.getTime()); /* Set Cookie to Expire Now */
			s_c_w( "s_traffic", "" , delDate, "/", ".lenovo.com" );
		}
	} catch(e){
	}
	
	/* Check for the setSynthTraffic flag. If present, set the s_traffic cookie with the given parameters */
	try{
		var synthTrafficTool = s_getQueryStr('setSynthTraffic');
		if( synthTrafficTool !== "" ){
				synthTrafficTool = synthTrafficTool.replace(/\W/g,"");
				var date = new Date();
				date.setTime(date.getTime()+(2592000000)); /* Set Cookie to Expire in 30 Days 30x24*60*60*1000 */
				/* Sets cookie.  */
				s_c_w( "s_traffic", synthTrafficTool , date, "/", ".lenovo.com" );
		}
	} catch(e){
	}
	
	/* Read the s_traffic cookie to determine if a custom report suite should be used */
	try{
		var synthTrafficCookie = s_c_r( "s_traffic" );
		if( typeof synthTrafficCookie !== "undefined" && synthTrafficCookie !== "" ){
			if( typeof window.lmd === "undefined" ){
				window.lmd = {};
			}
			window.lmd.cc = "ww";
			window.lmd.bu = "filter";
			window.lmd.storeID = synthTrafficCookie;
		}
	} catch(e){
	}
};
...
if(!s_duplicateLoad){
	...
	if( document.location.hostname.match( /^[0-9\.]+$/ ) || document.location.hostname.match( "localhost" ) || document.location.pathname.match( "/us/en/perf/hp/" ) ){
...
function s_doPlugins(s){
	...
	if(cT!='+'){ //if s.t or s.tl call...
		...
		s.prop22 = s_c_r('s_vi'); //SiteCatalyst VisitorID
		...
		if(s_errorPage || s.pageType){ //if we're on an error page...
			...
			if( typeof s_errorRef !== "undefined" && s_errorRef ) s.referrer = s_errorRef;
			else var s_errorRef = dREF || 'None';
		...
		/* FINAL pageName */
		if( (lmd.bu !== "support" && lmd.buOrig !== "support") && (lmd.bu !== "community" && lmd.buOrig !== "community") && (lmd.bu !== "forum") ){
		...
		if(s.url){
			var dataDownloadName = s.p_gh().getAttribute('data-download-name');
			var dataDownloadType = s.p_gh().getAttribute('data-download-type') || "unknown type";
			if(dataDownloadName){
			  s.eVar33 = s.prop33 = dataDownloadType + ":" + dataDownloadName + ":" + s.url.substring(s.url.lastIndexOf('/')+1,s.url.length); //Download Filename
			}
			else{
		...
		if( (lmd.bu === "support" || lmd.buOrig === "support") || (lmd.bu === "forum") ){
		...
		/* Eloqua */
		s.eVar28=s.getQueryParam('elq_smb_cid'); //Eloqua ID
		...
		if(!s.eVar9)s.eVar9=s.getQueryParam('rrid'); //Responsys RRID
		...
		*/
		...
		if( typeof _satellite !== "undefined" ){
			s.prop67 += '|DTM:' + _satellite.getVar( "dtm:propertyName" ) +'|';
		} else {
			s.prop67 += '|LEN|';
		}
		...
		/* ClickTale UID */
		s.eVar64 = _satellite.getVar( "clickTale:UID" );
...
try{
	...
	var b = _satellite.customGetVisitorID();
	if(b){
		lDil.api.aamIdSync({
			dpid: "929",
			dpuuid: b,
